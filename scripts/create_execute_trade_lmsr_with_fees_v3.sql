-- Create LMSR version of execute_trade function with fee handling
...
DECLARE
  ...
  v_fee_amount DECIMAL := p_bet_amount * 0.005; -- 0.5% fee
  v_net_amount DECIMAL := p_bet_amount - v_fee_amount;
BEGIN
  ...
  -- Update user balance (subtract full bet amount including fee)
  UPDATE profiles 
  SET balance = balance - p_bet_amount 
  WHERE id = p_user_id;
  
  -- Update market with LMSR parameters (using net amount for liquidity)
  UPDATE markets 
  SET 
    qy = p_qy,
    qn = p_qn,
  ...
  -- Create transaction record for the bet (full amount)
  ...
  VALUES (
    ...
    'bet', 
    p_bet_amount, 
    'Bet ' || p_bet_amount || ' on ' || p_bet_side || ' (LMSR) - Fee: $' || v_fee_amount
  );
